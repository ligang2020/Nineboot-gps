    ...
    private var settingsContent: some View {
        ScrollView {
            VStack(spacing: 14) {
                ...
                if !model.dashboard.vehicles.isEmpty {
                    VehicleNameSettingsCard(model: model)
                        .frame(maxWidth: .infinity, alignment: .leading)
                        .padding(16)
...
private struct VehicleNameSettingsCard: View {
    @ObservedObject var model: NinebotViewModel
    @State private var editingVehicle: NinebotVehicleSnapshot?
        ...
        VStack(alignment: .leading, spacing: 12) {
            HStack(alignment: .firstTextBaseline) {
                VStack(alignment: .leading, spacing: 3) {
                    Text("车辆名称")
                        .font(.headline)
                    Text("名称会同步用于主 App、小组件和灵动岛；不会修改车辆原始编号。")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
                Spacer(minLength: 8)
            ...
            ForEach(model.dashboard.vehicles) { snapshot in
                Button {
                    ...
                    HStack(spacing: 11) {
                        Image(systemName: "scooter")
                            ...
                            .background(Color.teslaGreen.opacity(0.12), in: Circle())
                        VStack(alignment: .leading, spacing: 2) {
                ...
                if snapshot.id != model.dashboard.vehicles.last?.id {
                    Divider()
                }
            }
        }
        .sheet(item: $editingVehicle) { snapshot in
            VehicleNameEditor(snapshot: snapshot) { name in
                model.saveVehicleDisplayName(name, for: snapshot.vehicle.sn)
            }
...
private struct VehicleNameEditor: View {
    var snapshot: NinebotVehicleSnapshot
    var onSave: (String) -> Void
    @Environment(\.dismiss) private var dismiss
    @State private var name: String

    init(snapshot: NinebotVehicleSnapshot, onSave: @escaping (String) -> Void) {
        self.snapshot = snapshot
        self.onSave = onSave
        _name = State(initialValue: NinebotVehicleNameResolver.alias(for: snapshot.vehicle.sn) ?? snapshot.vehicle.displayName)
    }
        ...
        NavigationStack {
            VStack(alignment: .leading, spacing: 18) {
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text("这个名称会用于仪表、小组件与实时活动。")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }

                TextField("例如：B2轰炸机", text: $name)
                ...
                Button(role: .destructive) {
                    ...
                    Label("恢复默认名称", systemImage: "arrow.counterclockwise")
                        .font(.subheadline.weight(.semibold))
                }
                .buttonStyle(.bordered)
    ...
    var body: some View {
        NavigationStack {
            VStack(alignment: .leading, spacing: 16) {
                ...
                SettingsInputField(
                    title: model.dataSourceMode == .platform ? "Bearer Token（可选）" : "访问口令（可选）",
                    placeholder: model.dataSourceMode.tokenPlaceholder,
                    systemImage: "key.horizontal.fill",
                    text: $model.bearerToken,
                    isSecure: true
    ...
    var body: some View {
        DisclosureGroup(isExpanded: $isExpanded) {
            VStack(alignment: .leading, spacing: 14) {
                ...
                SettingsInputField(
                    title: "九号账号",
                    placeholder: model.dataSourceMode == .platform ? "九号账号手机号" : "九号账号手机号",
...
private struct SettingsProfileCard: View {
    ...
    private var subtitle: String {
        let vehicleName = snapshot?.vehicle.displayName.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
...
private struct AppVersionFooter: View {
    private var versionText: String {
        let shortVersion = Bundle.main.object(forInfoDictionaryKey: "CFBundleShortVersionString") as? String ?? "1.2.48"
